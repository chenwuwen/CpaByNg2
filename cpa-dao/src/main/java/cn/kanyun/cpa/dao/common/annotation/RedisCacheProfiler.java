...
public @interface RedisCacheProfiler {
    ...
    String cacheKey(); //cacheKey() 缓存key的前缀，缓存key由key前缀+入参组成

    int expire() default 60; //expire() 缓存的过去时间，默认为60秒

    boolean readFromCache() default true;  //readFromCache() 是否读取缓存，默认为true
